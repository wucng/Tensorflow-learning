...
模型：卷积神经网络 
模型结构：卷积核 5x5 strides 1x1 ；池化核 2x2 strides 3x3 

第一个卷积池化层后，W(宽)变化: （28-5+2*2)/3+1=10
第二个卷积池化层后，W(宽)变化: （10-5+2*2)/3+1=4
最后图像卷积池化后 从 1x28x28 变成 cx4x4  （c为最后定义的通道数）
...
class Net(nn.Module): # 自定义网络模型
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 10, kernel_size=5) # 通道数 1-->10 (mnist 通道数 1) 卷积核大小 5x5
        self.conv2 = nn.Conv2d(10, 20, kernel_size=5) # 通道数 10-->20 (mnist 通道数 1) 卷积核大小 5x5
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 320)  # 将每张图片展开成一行，一行代表一张图像
        x = F.relu(self.fc1(x))
