...
class tf_tfrecord():
    ...
    def numpy_to_tfrecord(self):
        ...
        for i in range(len(self.data)):
            image_bytes = self.data[i].astype(np.float16).tobytes()
    ...
    def tfrecord_to_numpy(self,batch_size=2):
        ...
        image = tf.reshape(record_image, [1, 3])
    ...
    def Image_to_tfrecord(self,record_location,img_pixel):
        '''
        直接将图像转成tfrecord
        ...
        for image_path in images_path:
            ...
            if current_index % 100 == 0: # 每隔100幅图像，训练样本的信息就被写入到一个新的Tfrecode文件中，以加快操作的进程
                if writer:
                    writer.close()
    ...
    def tfrecord_to_numpy(self,img_pixel,batch_size=2):
        filename_queue = tf.train.string_input_producer(
            tf.train.match_filenames_once(self.file_path))  # 加载多个Tfrecode文件
        reader = tf.TFRecordReader()
        _, serialized = reader.read(filename_queue)
        ...
        image = tf.reshape(record_image, [img_pixel, img_pixel,3])
        # label = tf.cast(features['label'], tf.int64)
    ...
    def Image_processing(self,reshaped_image,height, width):
        '''
        图像预处理
    ...
    def Image_to_tfrecord_2(self,dir_name,img_pixel):
        writer = tf.python_io.TFRecordWriter(dir_name + "train.tfrecords")
    ...
    def Image_to_tfrecord_3(self,record_location,img_pixel,sess):
        '''
        ...
        for image_path in images_path:
            ...
            try:
                ...
                resized_image = tf.image.resize_images(image, (img_pixel, img_pixel))
