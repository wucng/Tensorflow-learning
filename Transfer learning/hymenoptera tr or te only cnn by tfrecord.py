...
# conv5
conv5 = conv2d(conv4,
               3, 128, 2, is_training,'conv5') # hymenoptera 为2类，只需对这一层参数进行修改，再训练即可
conv5 = tf.nn.max_pool(conv5, [1, 2, 2, 1], [1, 2, 2, 1], padding="VALID")  # [n,1,1,2]
...
# exclude = ['conv5/conv5/Relu:0','Reshape:0','norm4_1/','conv5/conv5/Relu:0',
#            'MaxPool_4:0','norm4_1:0','conv5/conv5/mul:0','conv5/conv5/add:0',
#            'conv5/b:0','conv5/b']
...
exclude=[]
exclude.append('Reshape:0')
exclude.append('norm4_1:0')
exclude.append('conv5/conv5/Relu:0')
exclude.append('MaxPool_4:0')
for key in var_to_shape_map:
    if 'conv5' in key:
        exclude.append(key+':0')

variables_to_restore = slim.get_variables_to_restore(exclude=exclude)

if __name__=="__main__":
    if train:
        img_batch, label_batch=load_images_from_tfrecord("output2/training-images/*.tfrecords",batch_size)
        with tf.Session(graph=tf.get_default_graph()) as sess:
            ...
            # tf.initialize_variables(conv5) # 只初始化输出层，其他使用cifar10训练的模型参数
        ...
        img_batch, label_batch = load_images_from_tfrecord("output2/testing-images/*.tfrecords", batch_size)
        with tf.Session(graph=tf.get_default_graph()) as sess:
            tf.global_variables_initializer().run()
            tf.local_variables_initializer().run()
            saver = tf.train.Saver()
